...
const StudioMulti = ({
  ...
  playScene  // Added for individual scene preview
}) => {
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    {/* Preview Button - Only show when scene is done */}
                    {scene.status === 'done' && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="p-1.5 rounded-lg transition-all bg-purple-600/20 hover:bg-purple-600/40 text-purple-400 hover:text-purple-300"
...
const App = () => {
  ...
  const [subtitleWordsPerLine, setSubtitleWordsPerLine] = useState(6);
  ...
  const playScene = async (sceneIndex) => {
    // Set active scene and play just that one scene
    setActiveSceneIndex(sceneIndex);
    setIsPlaying(true);

    const audio = sceneAudiosRef.current[sceneIndex];
    ...
    if (audio) {
      ...
      try {
        ...
        console.error("Audio play failed for scene", sceneIndex, err);
      }
      ...
      // No audio, just show image for 3 seconds
      const startTime = Date.now();
      await new Promise(resolve => {
        const loop = () => {
          if (!isPlaying) { resolve(); return; }
